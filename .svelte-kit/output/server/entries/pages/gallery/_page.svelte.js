...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `${validate_component(Navbar, "Navbar").$$render($$result, {}, {}, {})} <div class="gallery-content min-h-screen bg-gray-50"><div class="max-w-[2000px] mx-auto p-4"><div class="gallery-grid svelte-1ecjnii">${each(galleryItems, (item, index) => {
    return `<a${add_attribute("href", item.image, 0)} data-lightbox="gallery"${add_attribute("data-title", item.title, 0)} class="${"gallery-item " + escape(item.size, true) + " svelte-1ecjnii"}"><div class="relative overflow-hidden w-full h-full"><img${add_attribute("src", item.image, 0)}${add_attribute("alt", item.title, 0)} class="w-full h-full object-cover"> <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 hover:opacity-100 transition-all duration-300"><div class="absolute bottom-0 left-0 p-6"><span class="text-sm text-gray-300 mb-2 block">${escape(item.category)}</span> <h2 class="text-2xl font-bold text-white">${escape(item.title)}</h2></div> </div></div> </a>`;
